...
export function RandomMovement(ghost=new Ghost(0xfff,0,0), otherGhosts=[new Ghost(0xfff, 0, 0)], player = new Player(), gates=[new Gate(0,0,0,0)])
{
    ...
    const getPossibleMoves = () => {
        let possibleDirections = allDirections().filter(([x, z]) => {
            ...
            {
                if(pointIntersectsGates([x, z], gate))
                    return false;
            }
    ...
    const getBestMove = () => {
        ...
        if(possibleMoves.length == 0)
            return [ghost.x, ghost.z];
        if(ghost.previousDir)
        {
            let [prevPathX, prevPathZ] = ghost.previousDir;
            if(possibleMoves.indexOf([ghost.x + prevPathX, ghost.z + prevPathZ]) > -1)
            {
                if (Math.random() < 0.75)
                {
